...
### 22.13 W40-startcontrole en echt conceptmanifest — 26 september
Controle op kop `a6b7caf`: W40 is al geregistreerd als kandidaatweek voor **27 september t/m 3 oktober 2026**. Het oude document `RELEASE-GATE-2026-W40-CANDIDATE-SOURCE.md` beschrijft een achterhaalde bronblokkade: de actuele repository bevat inmiddels 85 kandidaatregels. De bestaande geïntegreerde capaciteitstest is opnieuw lokaal uitgevoerd met zijn eigen geïsoleerde testcontext: 85 unieke Spotify-ID's, 85 unieke artiestcredits, 63/63 unieke stresskeuzes, 22 over, nul overlap met de in die test opgenomen historische ID's. Dit bewijst geen actuele Spotify-beschikbaarheid of ratings; het label PRODUCTION-READY van deze test geldt alleen voor kandidaatcapaciteit.

Het bestaande manifesttemplate is geïnstantieerd als [W40-conceptmanifest](../data/week-publication-manifest-2026-40.json). Geen nieuwe architectuur. Het bevat Imperiale topaas, steen 5, de eerder gecontroleerde zakelijke museumtekst en het bestaande gegenereerde hoesbestand. Tracks blijven leeg, freeze blijft draft, productieplaylist-ID blijft null, attachedToSpotify blijft false en integrity.complete blijft false. De privéhoesproef wordt niet als echte weekpublicatie overgenomen.

Uitvoering met de gerepareerde weekmotor: artworkPreflight slaagt met teller 5; selectiepoort en productiehandoff blokkeren wegens ontbrekende weekinhoud/freeze; externalDeliveryReadiness blokkeert wegens ontbrekende productieplaylist-ID; totale weekpublicatie blijft incompleet. [Vastgelegde resultaten](W40-STARTCONTROLE-2026-09-26.json).

**Concreet resterend aansluitwerk:** `test/zaterdagcadeau-v3.html` laadt en herschrijft nog `zaterdagcadeau-v2.html`. Die route publiceert vanuit GIFT/localStorage en laadt de nieuwe weekmanifestmotor niet. `completeDelivery` markeert de tracklevering als complete zonder hoesupload; de huidige authorize-functie vraagt alleen playlist-modify-private. In deze onderzochte route ontbreekt een request naar het Spotify-playlist-images-endpoint. De repo-contractmotor en de werkelijke publicatiepagina zijn dus nog niet gekoppeld. Wachten op weekinhoud alleen lost dit technische gat niet op.

Volgende technische stap: de bestaande zaterdagroute voor W40 laten werken vanuit het geverifieerde, bevroren manifest en het daadwerkelijke hoesbestand, met upload en controle achteraf. Historische weken niet stil omzetten of repareren. Een volledige echte weekproef wacht daarnaast op Bens werkelijke weekbeoordelingen en definitieve 19+2-selectie. Deze startcontrole heeft geen Spotify-mutaties uitgevoerd.
