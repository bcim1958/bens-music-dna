...
const registry={
  ...
  entities:{
    ...
    "the_smiths":{type:"artist",name:"The Smiths"},
    "bert_jansch":{type:"person",name:"Bert Jansch"},
    ...
    "brian_eno":{type:"person",name:"Brian Eno"},
    "801":{type:"artist",name:"801"},
    "talking_heads":{type:"artist",name:"Talking Heads"},
    "david_byrne":{type:"person",name:"David Byrne"},
    "tina_weymouth":{type:"person",name:"Tina Weymouth"},
    "chris_frantz":{type:"person",name:"Chris Frantz"},
...
"independent-editorial":{status:"completed","note":"Rolling Stone, Sounds, Creem, Kerrang, Louder and other editorial coverage"},
"discography-credits":{status:"completed","note":"multi-era album and recording history established"},
"relationships-network":{status:"completed","note":"Rush, Celtic Frost, Coroner, Jason Newsted, Ozzy/Ozzfest, producers and influence network"},
...
"archive-secondary":{status:"completed","note":"official historical press archive preserves contemporary interviews"}
  ...
  sources:{
    ...
    allmusic_801_bio:{provider:"AllMusic",kind:"editorial-biography",title:"801 — Biography",url:"https://www.allmusic.com/artist/801-mn0000577733/biography",checkedAt:"2026-09-25"},
  ...
  discoveries:[
    ...
    {id:"disc-manzanera-801-eno",base:"phil_manzanera",counterpart:"801",kind:"spin-off-door",title:"De kleine deur naast Roxy Music",summary:"Naast de voor de hand liggende Roxy Music-route leidt Manzanera naar het kortstondige 801, waar Brian Eno opnieuw in beeld komt.",relations:["rel-manzanera-801","rel-eno-801"],evidence:["allmusic_801_bio"],status:"unread"},
...
function nextResearchTargets(limit=5){
  return researchBacklog().filter(x=>x.missingResearchPaths.length>0).slice(0,limit);
}
function researchTargetReason(id){
  const x=researchBacklog().find(y=>y.id===id); if(!x)return null;
  return {id:x.id,name:x.name,coverage:x.coverage,missing:x.missingResearchPaths,
    reason:x.coverage==="developing"?"close-evidence-gaps":x.coverage==="light"?"broaden-research-routes":"open-new-world"};
}
...
function researchOdometer(){
  ...
  const proven=s.classifiable;
  const explored=s.worlds.filter(w=>["well-researched","deep"].includes(w.coverage)).length;
  const frontier=s.worlds.filter(w=>["unresearched","light","developing"].includes(w.coverage)).length;
  const pct=n=>Math.round((n/total)*100);
  return {
    catalog:{artistWorlds:s.artistWorlds,explored,frontier,exploredPercent:pct(explored),frontierPercent:pct(frontier)},
    axis:{left:"proven-world",right:"unexplored-world",positionPercentFromLeft:pct(explored)},
    coverage:coverageOrder.map(state=>({state,count:s.coverage[state]||0,percent:pct(s.coverage[state]||0)})),
    richness:{
      eligibleWorlds:proven,
      excludedUntilResearched:s.artistWorlds-proven,
      distribution:Object.entries(s.richness).filter(([k])=>k!=="unknown").map(([state,count])=>({state,count}))
    },
    rule:"richness distribution excludes worlds whose research coverage is not yet sufficient"
  };
}
...
function ghostDiscoveryQueueRegressionSelfTest(){
  const state=createDiscoveryState();
  const q1=genericDiscoveryQueue("ghost",state,6);
  const q2=genericDiscoveryQueue("ghost",createDiscoveryState(),6);
  const counterparts=q1.map(discoveryCounterpartId);
  const ids=q1.map(d=>d.id);
  const voivod=q1.filter(d=>d.counterpart==="voivod");
  const cases=[
    {name:"Ghost queue has six fresh doors",pass:q1.length===6},
    {name:"one visible discovery per counterpart",pass:counterparts.length===new Set(counterparts).size},
    {name:"Voivod cluster contributes at most one door",pass:voivod.length<=1},
    {name:"queue is deterministic for identical state",pass:ids.join("|")===q2.map(d=>d.id).join("|")},
    {name:"new refreshed stock participates",pass:ids.some(id=>["disc-ghost-sabbath-beyond-doom","disc-ghost-mercyful-fate-foundation","disc-ghost-misfits-pop-darkness","disc-ghost-rammstein-scale-not-sound"].includes(id))}
  ];
...
function explorerWeekContext(weekKey,day){
  const week=window.MUSIC_DNA_WEEKS&&MUSIC_DNA_WEEKS.get?MUSIC_DNA_WEEKS.get(weekKey):null;
  if(!week)return {weekKey,ready:false,reason:"week-unavailable",todayArtistIds:[],weekArtistIds:[],latestWArtistIds:[]};
...
function explorerEntrypoints(context){
  ...
  return [
    {type:"free",label:"Vrij kiezen",mode:"search",items:[]},
...
function explorerEntrypointRegressionSelfTest(){
  ...
  const cases=[
    {name:"five equal entry types exist",pass:eps.length===5},
...
function explorerGhostVoivodCrossWorldRegressionSelfTest(){
  const walk=createExplorerWalk("ghost",{type:"free",label:"Vrij kiezen",id:"ghost"});
  const ghost=explorerNode("ghost");
  const ghostToVoivod=ghost&&ghost.links.find(x=>x.id==="voivod");
  const atVoivod=ghostToVoivod?explorerStep(walk,"voivod"):null;
  const voivodToRush=atVoivod&&atVoivod.links.find(x=>x.id==="rush");
  const atRush=voivodToRush?explorerStep(walk,"rush"):null;
  const breadcrumbAtRush=explorerBreadcrumb(walk);
  const backToVoivod=atRush?explorerBack(walk):null;
  const backToGhost=backToVoivod?explorerBack(walk):null;
  const ghostQueue=genericDiscoveryQueue("ghost",createDiscoveryState(),6);
  const voivodDoor=ghostQueue.find(d=>d.counterpart==="voivod");
  const cases=[
    {name:"Ghost exposes Voivod as one evidenced door",pass:!!ghostToVoivod&&ghostToVoivod.relationCount===1},
    {name:"Ghost discovery queue can select the Voivod door",pass:!!voivodDoor},
    {name:"Voivod exposes Rush as a real next world",pass:!!voivodToRush},
    {name:"walk reaches Rush through two evidenced edges",pass:!!atRush&&walk.trail.slice(0,3).join(">")==="ghost>voivod>rush"},
    {name:"breadcrumb preserves the cross-world route",pass:breadcrumbAtRush.map(x=>x.id).join(">")==="ghost>voivod>rush"},
    {name:"back navigation returns Voivod then Ghost",pass:!!backToVoivod&&!!backToGhost&&walk.cursor===0&&walk.trail[walk.cursor]==="ghost"}
  ];
  return {pass:cases.every(x=>x.pass),cases,route:["ghost","voivod","rush"],evidence:["rel-ghost-voivod-influence","rel-voivod-rush-influence"],invariant:"a discovery may open a new world, continue across another evidenced relation, and return through the exact breadcrumb without fabricating edges"};
}
