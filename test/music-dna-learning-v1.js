(function(){
  ...
  function readLearning(){return safeParse(window.localStorage.getItem(LEARNING_KEY),{version:5,signals:{},updatedAt:null});}
  ...
  function exactSpotifyUrl(id,t){
    ...
    if(!t)return null;
    if(t.spotifyUrl&&/open\.spotify\.com\/track\/[A-Za-z0-9]+/.test(t.spotifyUrl))return t.spotifyUrl;
  ...
  function upsertSignal(id,rating,ratedAt){
    if(!rating||!Object.prototype.hasOwnProperty.call(WEIGHTS,rating))return;var t=findTrack(id);if(!t)return;var url=exactSpotifyUrl(id,t);
    var model=readLearning();if(!model.signals)model.signals={};model.version=5;
  ...
  function hydrateExistingDelivery(){
    var model=readLearning(),changed=false;if(!model.signals)model.signals={};
    for(var id in model.signals){if(!Object.prototype.hasOwnProperty.call(model.signals,id))continue;var s=model.signals[id],url=exactSpotifyUrl(id,findTrack(id));if(url&&(s.spotifyUrl!==url||s.spotifyResolvedExact!==true)){s.spotifyUrl=url;s.spotifyResolvedExact=true;changed=true;}}
    if(changed){model.version=5;model.updatedAt=new Date().toISOString();writeLearning(model);}return changed;
  }
  ...
  hydrateExistingDelivery();
  var originalSetItem=window.localStorage.setItem.bind(window.localStorage);window.localStorage.setItem=function(key,value){originalSetItem(key,value);if(isStateKey(key))ingestState(value);};
  window.MUSIC_DNA_LEARNING={version:5,weights:WEIGHTS,storageKey:LEARNING_KEY,legacyKeys:LEGACY_KEYS.slice(),productionKeyPattern:PROD_RE,read:readLearning,hydrateExistingDelivery:hydrateExistingDelivery};
})();